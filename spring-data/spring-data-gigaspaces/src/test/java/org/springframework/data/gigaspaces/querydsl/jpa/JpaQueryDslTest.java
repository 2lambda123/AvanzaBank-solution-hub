...
public class JpaQueryDslTest extends BaseRepositoryTest {
  private static final BooleanExpression chrisIdPredicate =
      QPerson.person.id.eq(chris.getId());
  private static final BooleanExpression chrisPredicate =
      QPerson.person.name.eq(chris.getName());
  private static final BooleanExpression paulPredicate =
      QPerson.person.name.eq(paul.getName());
  private static final BooleanExpression chrisOrPaulPredicate =
      chrisPredicate.or(paulPredicate);

  @PersistenceContext private EntityManager entityManager;

  @Test
  public void testFindOneWithPredicate() {
    assertEquals(chris, new JPAQueryFactory(entityManager)
                            .from(QPerson.person)
  ...
  public void testFindTwoWithPredicate() {
    assertEquals(newHashSet(chris, chris2, chris3, paul, paul2),
