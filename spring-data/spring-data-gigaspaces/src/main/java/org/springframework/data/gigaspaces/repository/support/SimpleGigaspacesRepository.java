 ...
 */
public class SimpleGigaspacesRepository<T, ID extends Serializable>
    implements GigaspacesRepository<T, ID> {
  protected GigaSpace space;
  protected EntityInformation<T, ID> entityInformation;

  public SimpleGigaspacesRepository(
      GigaSpace space, EntityInformation<T, ID> entityInformation) {
    this.space = space;
  ...
  @Override
  public <S extends T> LeaseContext<S> save(S entity, long lease,
                                            TimeUnit unit) {
  ...
  @Override
  public <S extends T> Iterable<LeaseContext<S>>
  save(Iterable<S> entities, long lease, TimeUnit unit) {
    Assert.notNull(unit, "unit must not be null");
    LeaseContext<S>[] leaseContexts =
  ...
  private Iterable<T> findAllByIdsInternal(Iterable<ID> ids,
  ...
  private List<T> findAllSortedInternal(Projection projection, Sort sort,
                                        Pageable pageable) {
  ...
  public void deleteAllById(Iterable<? extends ID> ids) {
    // TODO: consider replacing with distributed task to not return deleted
    // entities back on client
